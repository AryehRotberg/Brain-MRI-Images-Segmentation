...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="An evaluation pipeline for brain MRI segmentation."
    )
    parser.add_argument(
        "--images_path", type=str, help="A directory that contains images only."
    )
    parser.add_argument(
        "--masked_images_path",
        type=str,
        help="A directory that contains masked images only.",
    )
    parser.add_argument("--model_path", type=str, help="A path to a trained model.")
    args = parser.parse_args()

    _, _, test_loader = DataTransformation(
        args.images_path, args.masked_images_path
    ).get_data_loaders(data_directory="outputs/data")
    ...
    loss = model_evaluator.calculate_loss(test_loader)

    logger.info(f"Loss of {test_loader = }: {loss}")

    model_evaluator.get_iou_dataframe("outputs/iou_df.csv")

    logger.info(f"Saved IOU dataframe to outputs directory.")

    model_evaluator.get_confusion_matrix(
        "outputs/confusion_matrix.csv", "outputs/confusion_matrix.png"
    )

    logger.info(f"Saved confusion matrix to outputs directory.")

    model_evaluator.get_classification_report("outputs/classification_report.csv")

...
class ModelEvaluation:
    ...
    def get_iou_dataframe(self, output_path: str) -> pd.DataFrame:
        ...
        for image_path in tqdm(images_list):
            prediction = self.predictor.predict_and_transform(
                os.path.join(self.images_path, image_path)
            )
            mask_gt = np.array(
                Image.open(os.path.join(self.masked_images_path, image_path)).convert(
                    "L"
                )
            )
        ...
        iou_df = pd.DataFrame(
            {"image_path": images_list, "tumor": self.val_df.tumor, "iou": iou_list}
        )
    ...
    def get_classification_report(self, output_path: str):
        ...
        report = (
            pd.DataFrame(
                classification_report(self.val_df.tumor, predictions, output_dict=True)
            )
            .round(2)
            .transpose()
        )
